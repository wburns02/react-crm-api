...
@app.post("/health/db/migrate-benefits-109")
async def run_benefits_109_migration():
    """Apply migration 109 (integrations + deductions) + seed demo rows."""
    from sqlalchemy import text
    from app.database import async_session_maker
    import os
    import subprocess
    ...
    try:
        async with async_session_maker() as session:
            ...
            try:
                row = await session.execute(
                    text(
                        "SELECT to_regclass('public.hr_benefit_carrier_integrations') IS NOT NULL"
                    )
                )
                results["table_exists_before"] = bool(row.scalar())
            except Exception:
        ...
        os.chdir("/app")
        stamp_target = "109" if results.get("table_exists_before") else "108"
        proc = subprocess.run(
            ["alembic", "stamp", stamp_target],
            capture_output=True,
            text=True,
        ...
        if results["upgrade"]:
            from app.hr.benefits.seed import seed_integrations_and_deductions
            async with async_session_maker() as session:
                results["seed"] = await seed_integrations_and_deductions(session)
        ...
        async with async_session_maker() as session:
            try:
                ...
                results["version_after"] = None
            try:
                row = await session.execute(text("SELECT count(*) FROM hr_benefit_scheduled_deductions"))
                results["scheduled_deductions_count"] = row.scalar_one()
            except Exception:
                results["scheduled_deductions_count"] = None
            try:
                row = await session.execute(text("SELECT count(*) FROM hr_benefit_carrier_integrations"))
                results["integrations_count"] = row.scalar_one()
            except Exception:
                results["integrations_count"] = None
    except Exception as e:
        results["errors"].append(f"{type(e).__name__}: {e}")
...
@app.post("/health/db/migrate-payroll-113")
async def run_payroll_113_migration():
    from sqlalchemy import text
    from app.database import async_session_maker
    import os, subprocess
    results: dict = {"stamp": False, "upgrade": False, "seed": None, "errors": []}
    try:
        async with async_session_maker() as session:
            try:
                ...
                results["version_before"] = None
            try:
                row = await session.execute(text("SELECT to_regclass('public.hr_pay_runs') IS NOT NULL"))
                results["table_exists_before"] = bool(row.scalar())
            except Exception:
                results["table_exists_before"] = None
        os.chdir("/app")
        stamp_target = "113" if results.get("table_exists_before") else "112"
        proc = subprocess.run(["alembic", "stamp", stamp_target], capture_output=True, text=True, timeout=60)
        results["stamp"] = proc.returncode == 0
        results["stamp_target"] = stamp_target
        if stamp_target == "112":
        ...
        if results["upgrade"]:
            ...
            async with async_session_maker() as session:
                results["seed"] = await seed_payroll_demo(session)
        async with async_session_maker() as session:
            try:
                row = await session.execute(text("SELECT version_num FROM alembic_version"))
                results["version_after"] = row.scalar_one_or_none()
                ...
                results["version_after"] = None
            try:
                row = await session.execute(text("SELECT count(*) FROM hr_pay_runs"))
                results["pay_runs_count"] = row.scalar_one()
            except Exception:
                results["pay_runs_count"] = None
            try:
                row = await session.execute(text("SELECT count(*) FROM hr_payroll_people_status"))
                results["people_count"] = row.scalar_one()
            except Exception:
                results["people_count"] = None
    except Exception as e:
        results["errors"].append(f"{type(e).__name__}: {e}")
    return results
    ...
    try:
        async with async_session_maker() as session:
            try:
                ...
                results["version_before"] = None
            try:
                row = await session.execute(text("SELECT to_regclass('public.hr_cobra_enrollments') IS NOT NULL"))
                results["table_exists_before"] = bool(row.scalar())
            except Exception:
                results["table_exists_before"] = None

        os.chdir("/app")
        stamp_target = "111" if results.get("table_exists_before") else "110"
        proc = subprocess.run(["alembic", "stamp", stamp_target], capture_output=True, text=True, timeout=60)
        results["stamp"] = proc.returncode == 0
        results["stamp_target"] = stamp_target
        ...
        if results["upgrade"]:
            ...
            async with async_session_maker() as session:
                results["seed"] = await seed_cobra_demo(session)
        ...
        async with async_session_maker() as session:
            try:
                ...
                results["version_after"] = None
            try:
                row = await session.execute(text("SELECT count(*) FROM hr_cobra_enrollments"))
                results["enrollments_count"] = row.scalar_one()
            except Exception:
                results["enrollments_count"] = None
    except Exception as e:
