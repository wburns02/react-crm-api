        ...
        "rates": [
            {
                ...
                "overtime_rate": float((r.hourly_rate or 0) * (r.overtime_multiplier or 1.5)),
                "overtime_multiplier": float(r.overtime_multiplier) if r.overtime_multiplier else 1.5,
                "salary_amount": float(r.salary_amount) if r.salary_amount else None,
                "commission_rate": float(r.job_commission_rate or 0),
                "job_commission_rate": float(r.job_commission_rate) if r.job_commission_rate else None,
                "upsell_commission_rate": float(r.upsell_commission_rate) if r.upsell_commission_rate else None,
                "weekly_overtime_threshold": float(r.weekly_overtime_threshold) if r.weekly_overtime_threshold else 40.0,
...
# Period detail catch-all - MUST be LAST route in file to avoid catching
# /commissions/stats, /commissions/leaderboard, etc. as period_id
