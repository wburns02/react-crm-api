...
@router.post("/seed-steps", status_code=status.HTTP_200_OK)
async def seed_journey_steps(
    ...
    for journey in journeys:
        # Skip if journey already has steps
        if journey.steps and len(journey.steps) > 0:
            logger.info(f"Journey '{journey.name}' already has {len(journey.steps)} steps, skipping")
            continue

        # Get appropriate steps for this journey type
