...
def build_pipeline(*, session, websocket, context_aggregator_pair) -> Pipeline:
    ...
    # NOTE on sample rate: setting sample_rate=8000 broke Pipecat's soxr
    # resampler ("buffer size must be a multiple of element size") and
    # silenced the agent entirely on test call #5. Leave sample_rate
    # unset so Pipecat picks its default and handles all conversion to
    # the 8kHz μ-law that Twilio expects via TwilioFrameSerializer.
