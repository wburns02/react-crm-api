"""System prompt for the work voice agent — Phil / MAC Septic.

This file contains ONLY the work persona. The demo persona (Will Burns
alter-ego for ragging on brothers/friends) lives in
``app/services/voice_agent/demo_prompt.py`` so the two prompts physically
cannot bleed into each other. ``render()`` here dispatches to the demo
module when ``VOICE_AGENT_PERSONA=demo``.
"""
...
def render(prospect_context: str, persona: str | None = None) -> str:
    """Fill a system prompt with prospect-specific context.

    Dispatches between two completely separate persona modules:
      - "septic" (default) → SYSTEM_PROMPT in this file (Phil, MAC Septic)
      - "demo"             → demo_prompt.render() (Will Burns alter-ego)
    The two prompts live in different files so they cannot bleed into each other.
    """
    from app.config import settings
    chosen = persona or getattr(settings, "VOICE_AGENT_PERSONA", None) or "septic"
    if chosen == "demo":
        from app.services.voice_agent import demo_prompt
        return demo_prompt.render(prospect_context)
